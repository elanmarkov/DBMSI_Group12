...
public class IndexUtils {
	...
	public static IndexFileScan ZTree_scan(CondExpr[] selects, IndexFile indFile)
		...
		if (selects == null || selects[0] == null) {
			indScan = ((ZCurve) indFile).newZFileScan(null, null);
			return indScan;
		}
		...
		if(selects[1]!=null && selects[0].distance ==-1){
			if (selects[1].type1.attrType != AttrType.attrSymbol) {
				key2 = getValue(selects[1], selects[1].type1, 1);
			} else {
				key2 = getValue(selects[1], selects[1].type2, 2);
		...
		case AttrType.attrDesc:
			if(key2==null){
				indScan = ((ZCurve) indFile).ZFileRangeScan(key1, (int)selects[0].distance);
			}else{
				StringKey k1 = Utility.conver5Dto1D(key1);
				StringKey k2 = Utility.conver5Dto1D(key2);
				if (((StringKey) k1).getKey().compareTo(((StringKey) k2).getKey()) < 0) {
					indScan = ((ZCurve) indFile).newZFileScan(key1, key2);
				} else {
					indScan = ((ZCurve) indFile).newZFileScan(key2, key1);
				}
			}
			return indScan;
