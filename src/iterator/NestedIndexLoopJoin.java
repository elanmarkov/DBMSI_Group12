...
public class NestedIndexLoopJoin extends Iterator{
		...
		/**
		 * Iterates through every tuple on the outer relation, and
		 * uses an index look up on the inner tuple to make sure only tuples which satisfy
		 * the join condition are returned. Along with that, 
		 * a separate filter is applied on the attribute to make sure that only the tuple
		 * which satisfy the conditions are matched. In case of path expression, 
		 * it's the destination label.
		 * , and joins them
